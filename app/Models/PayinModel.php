...
class PayinModel extends Model
{
    ...
    public function getWalletDataAjax($start,$length,$date="",$search="",$userId=""){
        ...
        $selectedColumns = ['payin_transactions.payerName','payin_transactions.payerMobile','payin_transactions.orderId','payin_transactions.txnId','payin_transactions.amount','payin_transactions.charge','payin_transactions.gst','payin_transactions.totalAmount','payin_transactions.utr','payin_transactions.status','payin_transactions.created_at','users.name'];
        ...
        if($search != ""){
            $query->where('payin_transactions.txnId', 'like', '%' . $search . '%');
            $query->orWhere('payin_transactions.orderId', 'like', '%' . $search . '%');
            $query->orWhere('payin_transactions.utr', 'like', '%' . $search . '%');
            $query->orWhere('payin_transactions.amount', 'like', '%' . $search . '%');
            $query->orWhere('payin_transactions.charge', 'like', '%' . $search . '%');
            $query->orWhere('payin_transactions.gst', 'like', '%' . $search . '%');
            $query->orWhere('payin_transactions.totalAmount', 'like', '%' . $search . '%');
            $query->orWhere('payin_transactions.status', 'like', '%' . strtoupper($search) . '%');
