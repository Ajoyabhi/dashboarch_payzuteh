...
class PayoutModel extends Model
{
    ...
    public function getWalletDataAjax($start,$length,$date="",$search="",$userId=""){
        ...
        if($search != ""){
            $query->where('payout_transactions.txnId', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.orderId', 'like', '%' . $search . '%');
            $query->orWhere('users.name', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.utr', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.beneName', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.beneAccount', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.beneIfsc', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.amount', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.gst', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.totalAmount', 'like', '%' . $search . '%');
            $query->orWhere('payout_transactions.status', 'like', '%' . strtoupper($search) . '%');
