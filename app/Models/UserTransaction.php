...
class UserTransaction extends Model
{
    ...
    public function getChargebackDataAjax($start,$length,$date="",$search="",$userId=""){
        ...
        if($search != ""){
            $query->where('user_transactions.txnId', 'like', '%' . $search . '%');
            $query->orWhere('user_transactions.orderId', 'like', '%' . $search . '%');
            $query->orWhere('users.name', 'like', '%' . $search . '%');
            $query->orWhere('user_transactions.remark', 'like', '%' . $search . '%');
            $query->orWhere('user_transactions.openBalance', 'like', '%' . $search . '%');
            $query->orWhere('user_transactions.amount', 'like', '%' . $search . '%');
            $query->orWhere('user_transactions.walletBalance', 'like', '%' . $search . '%');
            $query->orWhere('user_transactions.status', 'like', '%' . strtoupper($search) . '%');
            //$query->where('user_transactions.txnId', $search)->orWhere('user_transactions.orderId', $search);
        }
        return $query->skip($start)->take($length)->orderBy('user_transactions.id','desc');
    }
