...
class PayoutHaodaPay extends Controller
{
    public function doPayout(Request $request)
    {
        ...
        // $payouts_status = $getUser->payouts_status;
        // if(payouts_status == 0){
        //     $responseData = [                
        //         'status' => FALSE,
        //         'error'=> TRUE,
        ...
        if(empty($getCommission)){
            ...
            $charge = $getCommission->payout_total_charge;
        ...
        // new added for update wallet balance of the user
        $user_wallet_bal = $getUser->wallet;
        $getUser->wallet = $user_wallet_bal - $totalDeductAmount;
        $getUser->updated_at = Carbon::now();
        $getUser->save();
        ...
        if($openBal == 0 || $openBal < $checkAmount){           
            $responseData = [                
                'status' => FALSE,
                'error'=> TRUE,
                'message' => 'Insufficient fund'.$checkAmount.'~'.$openBal,
            ];
            return response()->json($responseData , 200);
        }
        ...
        $payout_id = '';
        if(isset($responseData['status']) && $responseData['status'] == false && isset($responseData['error']) && $responseData['error'] == true) {
            return response()->json($responseData , 200);
        } else if(isset($responseData['status']) && $responseData['status'] == "Processing") {
            $payout_id = $responseData['payout_id'];
            return response()->json($responseData , 200);
        } else if(isset($responseData['status']) && $responseData['status'] == "server_busy") {
            return response()->json($responseData , 200);
        }  else if(isset($responseData['status']) && $responseData['status'] == "server_busy") {
            return response()->json($responseData , 200);
        } 
    ...
    public function checkCallbackData()
    {
        ...
        if($requestId != ''){
                ...
                if(!empty($checkTxn)){
                    ...
                    /*email mobile added */
                $responseData['mobile'] = $userData->mobile;
                $responseData['email'] = $userData->email;
                $sendCallbackData = json_encode($responseData);//passed as it is coming
